...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, const bool is_global_config)
{
    ...
    // ORCA_BELT: keep enable_support TICKABLE on belt printers — the user's
    // toggle gates the belt-specific preprocessor (validation/support_preprocess.py)
    // via Plater::belt_supports_preprocess_mode. The native OrcaSlicer support
    // generator (which would emit virtual-space columns failing gate R7) is
    // neutralised separately at slice time: belt_supports_inject_volumes sets
    // per-object enable_support=false right after injecting the model-space
    // support+wedge volumes, and per-object overrides beat the print-preset
    // value in the slicer pipeline. So ticking the box on belt = preprocessor
    // ON; native generator stays inert regardless. Disabling the field here
    // (as a previous version did) made the gate impossible to trigger.
