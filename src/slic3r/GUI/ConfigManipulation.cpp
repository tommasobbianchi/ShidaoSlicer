...
void ConfigManipulation::toggle_print_fff_options(DynamicPrintConfig *config, const bool is_global_config)
{
    ...
    // ORCA_BELT: Disable built-in support for belt printers.
    // Belt printers require model-space supports generated by support_preprocess.py.
    // Built-in OrcaSlicer support generates virtual-space columns that fail gcode gate R7.
    {
        auto printer_structure_opt = preset_bundle->printers.get_edited_preset().config.option<ConfigOptionEnum<PrinterStructure>>("printer_structure");
        bool is_belt = printer_structure_opt && printer_structure_opt->value == PrinterStructure::psBelt;
        if (is_belt)
            toggle_field("enable_support", false);
    }
