...
void MainFrame::show_device(bool bBBLPrinter) {
    ...
    if (bBBLPrinter) {
        ...
        // Restore upstream's PrinterWebView Device tab — it embeds Fluidd/Mainsail
        // inline in Orca, which is what users actually want. PrinterWebView is
        // lazily created and wrapped in try/catch so a WebKit backend failure
        // (the SIGSEGV inside libjavascriptcoregtk we saw on 2026-04-22) keeps
        // Orca alive rather than taking the whole app down.
        ...
        if (m_printer_view) {
            ...
            // Kick the URL load so Fluidd/Mainsail appears on first show.
            load_printer_url();
        }
    }
}
