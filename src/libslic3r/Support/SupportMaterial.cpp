...
// ===========================================================================
// ORCA_BELT: native belt-floor support clipping (opt-in, CLASSIC/grid only).
// Ported (grid-only) from upstream PR #12998. ADDITIVE: only active when the
// user selects belt_support_mode = NativeClipped. The default Preprocessor
// path (external support_preprocess.py + Plater no-fallthrough hard-stop) is
// 100% unchanged. UNVALIDATED — must pass belt_gcode_gate.py + HW print.
// TreeSupport* is intentionally NOT touched.
// ===========================================================================
...
// This is the region where the belt surface exists (the "phantom top surface").
// Returns empty if belt floor native clipping is not active.
static Polygons belt_floor_surface_polygon(
    const SlicingParameters &slicing_params,
    const PrintConfig       &print_config,
    coordf_t                 print_z)
{
    if (!belt_floor_native_clipped_active(slicing_params, print_config))
        return {};
...
// This is the region where support is allowed to exist (above the belt floor).
...
// For each support layer, subtract the belt floor half-plane at that layer's
// print_z from the support polygons. Same diff() pattern as
// trim_support_layers_by_object() so derived interface layers inherit it.
...
void PrintObjectSupportMaterial::generate(PrintObject &object)
{
    ...
    // ORCA_BELT: clip top contacts against the 45° belt floor (no-op unless NativeClipped).
    trim_support_layers_by_belt_floor(m_slicing_params, *m_print_config, top_contacts);
...
// ORCA_BELT: detect bottom contacts where support meets the 45° belt floor plane.
// Modeled on detect_bottom_contacts() but uses the belt-floor half-plane polygon
// instead of stTop object surfaces. Only meaningful when NativeClipped is active
// (caller gates on has_belt_floor). NOTE: our SlicingParameters uses
// `soluble_interface` where upstream PR uses `zero_gap_interface_bottom` — same
// "is there a gap below the bottom contact" semantics (soluble => zero gap).
    ...
    const Polygons                                   &supports_projected)
{
    ...
    Polygons belt_surface = belt_floor_surface_polygon(slicing_params, print_config, layer.print_z);
    ...
    SupportGeneratorLayerStorage &layer_storage, std::vector<Polygons> &layer_support_areas) const
{
    ...
    // ORCA_BELT: only active for belt + NativeClipped opt-in.
    const bool has_belt_floor  = belt_floor_native_clipped_active(m_slicing_params, *m_print_config);
        ...
        if (! overhangs_for_bottom_contacts.empty())
            ...
            task_group.run([this, &object, &layer, &top_contacts, contact_idx, &layer_storage, &layer_support_areas, &bottom_contacts, &overhangs_for_bottom_contacts, has_belt_floor
                ...
                , iRun, &polygons_new
                    ...
                    // ORCA_BELT: phantom belt-floor bottom contacts where support meets the belt plane.
                    if (has_belt_floor) {
...
//        float column_propagation_filtering_radius = scaled<float>(0.8 * 0.5 * (m_support_params.support_material_flow.spacing() + m_support_params.support_material_flow.width()));
        ...
        // ORCA_BELT: clip projections and support areas so support doesn't
        // propagate below the belt plane (no-op unless NativeClipped active).
        if (has_belt_floor) {
            Polygons valid_region = belt_floor_valid_region_polygon(
                m_slicing_params, *m_print_config, layer.print_z);
    ...
    // ORCA_BELT: trim belt-floor bottom contacts by the belt plane too.
    trim_support_layers_by_belt_floor(m_slicing_params, *m_print_config, bottom_contacts);
    ...
    const std::vector<Polygons> &layer_support_areas) const
{
    ...
    // ORCA_BELT: clip intermediate/base layers by the belt floor (no-op unless NativeClipped).
    // Interface layers derived from these intermediates inherit the trim.
    trim_support_layers_by_belt_floor(m_slicing_params, *m_print_config, intermediate_layers);
}
