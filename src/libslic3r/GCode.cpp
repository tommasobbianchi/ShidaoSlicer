...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail){   
        ...
        // ORCA_BELT: Emit belt printer headers for IdeaMaker/preview compatibility
        // Use the same broad belt detection as the rest of the codebase:
        // printer_structure may not propagate correctly, so also check the bool flags.
        if (print.config().printer_structure.value == PrinterStructure::psBelt ||
            print.config().printer_is_belt.value ||
            print.config().belt_inclined_gcode.value) {
            double belt_angle = print.config().belt_angle.value;
            // Get bed dimensions from printable_area
            auto bed_points = print.config().printable_area.values;
            double bed_x = 250.0, bed_y = 9999.0, bed_z = 250.0; // Defaults
            if (bed_points.size() >= 4) {
                bed_x = std::max(bed_points[1].x() - bed_points[0].x(), 250.0);
                bed_y = std::max(bed_points[2].y() - bed_points[0].y(), 2000.0);
                if (bed_y > 2000) bed_y = 9999.0; // Treat as infinite
            }
            file.write_format(";Belt Printer: 1\n");
            file.write_format(";Dimension: %.3f %.3f %.3f 0.400\n", bed_x, bed_y, bed_z);
            file.write_format(";Plate Shape: 0\n");
            file.write_format(";Origin Center: 0\n");
            file.write_format(";Belt Base Y: 0.000\n");
            file.write_format(";Belt Base Z: 0.010\n");
            file.write_format(";Belt Offset Y: 0.000\n");
            file.write_format(";Belt Offset Z: 0.000\n");
            file.write_format(";Belt Gantry Angle: %.0f\n", belt_angle);
            file.write_format(";Belt Remove Z: 0.283\n");
            file.write_format(";Belt Repetition: 1\n");
        }
    ...
    // ORCA: resonance‑avoidance on short external perimeters
{
    ...
    if (!variable_speed) {
        ...
        gcode += m_writer.set_speed(F, "", comment);
        {
            ...
            // Attention: G2 and G3 is not supported in spiral_mode mode or belt inclined mode (Z varies with Y)
            if (!m_config.enable_arc_fitting || path.polyline.fitting_result.empty() || m_config.spiral_mode || sloped != nullptr || m_belt_inclined_gcode) {
        ...
        for (size_t i = 1; i < new_points.size(); i++) {
            ...
            if (sloped == nullptr) {
                // Normal extrusion
                if (m_belt_inclined_gcode) {
                    double inclined_z = compute_belt_inclined_z(p, m_nominal_z);
                    gcode += m_writer.extrude_to_xyz(Vec3d(p.x(), p.y(), inclined_z), dE, GCodeWriter::full_gcode_comment ? tempDescription : "");
                } else {
