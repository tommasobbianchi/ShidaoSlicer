...
def run_slice():
    ...
    configs = [
        str(TEST_CONFIG_MACHINE),
        str(TEST_CONFIG_PROCESS),
        str(TEST_CONFIG_FILAMENT)
    ]
    
    cmd = [
        str(ORCA_SLICER),
        str(TEST_STL),
        "--load-settings", ";".join(configs),
        "--slice", "0",
...
def validate_coordinates():
    ...
    # Extract first layer moves
    moves = re.findall(r'G1 X([\d\.\-]+) Y([\d\.\-]+) Z([\d\.\-]+)', gcode)
    
    if not moves:
        print("❌ No G1 moves found in G-code")
        return False
    
    # For CR30 (Z-belt, 45°): 
    #  - X coords should be ~-10 to +10 (cube width)
    #  - Y should increase with layers (Zv mapped to Y)
    #  - Z stable or slow increase (Yv belt travel)
    ...
    print(f"\n📊 Coordinate Analysis (first 100 moves):")
    ...
    if y_range > 5:  # Layers accumulating in Y
        print("  ✅ Y increases with layers (V→F mapping OK)")
        checks_passed += 1
    else:
        print("  ❌ Y not increasing (V→F mapping issue?)")
    
    if z_range < 10:  # Belt travel shouldn't dominate
        print("  ✅ Z stable (belt travel minimal)")
        checks_passed += 1
    else:
        print("  ❌ Z range too large")
...
def validate_layer_count():
    ...
    if layer_count >= expected_layers * 0.9:  # Allow 10% tolerance
        print(f"  ✅ Layer count reasonable (within 10% of expected)")
        return True
    else:
...
def main():
    print("=" * 60)
    print("IT01: Belt Slicing Integration Test")
    print("=" * 60)
